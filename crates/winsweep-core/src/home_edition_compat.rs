...
impl HomeEditionCompat {
    ...
    pub fn get_elevation_requirement(&self, operation: &str) -> ElevationRequirement {
        match operation {
            ...
            "system_temp_cleanup" | "prefetch_cleanup" | "service_management" if self.is_home_edition => {
    ...
    pub fn is_operation_supported(&self, operation: &str) -> bool {
        !matches!(self.get_elevation_requirement(operation), ElevationRequirement::HomeNotSupported)
    }
    ...
    pub fn get_feature_info(&self, operation: &str) -> FeatureInfo {
        ...
        let (available, unavailable_reason, workaround) = match operation {
            "group_policy_cleanup" if self.is_home_edition => {
                (false, Some("Group Policy Editor is not available on Windows Home edition".to_string()), 
                 Some("Use registry edits or third-party tools like Policy Plus".to_string()))
            }
            "windows_update_cleanup" if self.is_home_edition => {
                (true, None, Some("Requires administrator privileges".to_string()))
            }
            "service_management" if self.is_home_edition => {
                (true, None, Some("Limited service management on Home edition".to_string()))
            }
            _ => (true, None, None),
        };
        ...
        match output {
            Ok(result) => {
                if result.status.success() {
                    ...
                    warn!("diskpart compaction failed: {}", 
    ...
    fn find_wsl_vhdx_path(&self, distribution: &str) -> Result<PathBuf> {
        ...
        if let Ok(path) = self.windows_api.read_registry_string(
            &format!(r"SOFTWARE\Microsoft\Windows\CurrentVersion\Lxss\{}", distribution),
            "VhdxFilePath",
        ) {
        ...
        let possible_paths = [
            format!(r"{}\AppData\Local\Packages\{}\LocalState\ext4.vhdx", 
                user_profile, self.get_wsl_package_name(distribution)),
            format!(r"{}\AppData\Local\WSL\{}\ext4.vhdx", user_profile, distribution),
        ];
        ...
        Err(anyhow::anyhow!("Could not find VHDX path for distribution {}", distribution))
    }
...
Note: On Windows Home edition, diskpart is the recommended method for VHDX compaction."#,
    ...
    fn detect_home_edition(windows_api: &WindowsApi) -> Result<bool> {
        if let Ok(edition) = windows_api.read_registry_string(
