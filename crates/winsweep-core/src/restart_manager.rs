...
use windows::Win32::Foundation::{CloseHandle, ERROR_SUCCESS, GetLastError, HANDLE, HRESULT};
use windows::Win32::System::RestartManager::{
    RmStartSession, RmRegisterResources, RmGetList, RmShutdown,
    RmRestart, RmEndSession, CCH_RM_SESSION_KEY, CCH_RM_MAX_APP_NAME,
    CCH_RM_MAX_SVC_NAME,
    RM_SESSION_KEY, RM_APP_STATUS, RM_REBOOT_REASON,
    RM_SHUTDOWN_TYPE, RM_START_PHASE, RM_APP_TYPE,
};
...
impl RestartManager {
    ...
    pub fn get_applications(&mut self) -> Result<Vec<RestartApplication>> {
        ...
        if result != ERROR_SUCCESS && result != 234 { // 234 = ERROR_MORE_DATA
            return Err(anyhow::anyhow!(
...
fn from_wide_array(wide_array: &[u16]) -> String {
    let end = wide_array.iter().position(|&c| c == 0).unwrap_or(wide_array.len());
