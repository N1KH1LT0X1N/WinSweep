...
impl PipManager {
    ...
    async fn get_env_paths(&self) -> Result<Vec<PathBuf>> {
        ...
        if let Some(home_dir) = dirs::home_dir() {
            ...
            paths.push(home_dir.join("AppData").join("Local").join("Programs").join("Python"));
        }
            ...
            match output {
                Ok(result) => {
                    if result.status.success() {
                        ...
                        warn!("pip cache purge failed: {}", String::from_utf8_lossy(&result.stderr));
                    }
                }
                Err(e) => {
            ...
            if path.exists() {
                ...
                match safe_delete_directory(&path).await {
                    Ok(size) => {
                        space_freed += size;
                        items_deleted += 1;
                        debug!("Deleted pip cache: {} (freed {})", path.display(), format_bytes(size));
                    }
                    Err(e) => {
