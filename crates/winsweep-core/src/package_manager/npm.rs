...
impl NpmManager {
    ...
    async fn get_global_modules_path(&self) -> Result<PathBuf> {
        ...
        // Fallback to default location
        Ok(home_dir.join("AppData").join("Roaming").join("npm").join("node_modules"))
    }
}
            ...
            match output {
                Ok(result) => {
                    if result.status.success() {
                        ...
                        warn!("npm cache clean failed: {}", String::from_utf8_lossy(&result.stderr));
                    }
                }
                Err(e) => {
            ...
            if path.exists() {
                ...
                match safe_delete_directory(&path).await {
                    Ok(size) => {
                        space_freed += size;
                        items_deleted += 1;
                        debug!("Deleted npm cache: {} (freed {})", path.display(), format_bytes(size));
                    }
                    Err(e) => {
