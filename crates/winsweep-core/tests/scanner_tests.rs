...
use winsweep_core::scanner::{ScanItem, Scanner};
...
async fn test_scan_empty_directory() {
    ...
    scanner
        .scan_directory(temp_dir.path(), &mut |item| {
...
async fn test_scan_performance() {
    ...
    let mut count = 0;
    scanner
        .scan_directory(temp_dir.path(), &mut |_| {
            count += 1;
            true
        })
        .await
...
async fn test_scan_error_handling() {
    ...
    let result = scanner
        .scan_directory(&PathBuf::from("/nonexistent/path"), &mut |_| true)
        .await;
