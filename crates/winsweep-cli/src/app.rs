...
use winsweep_core::{WindowsEditionDetector, WslDetector, HomeEditionCompat, WslVersion, WslState, PackageManagerRegistry, DockerClient, ContainerInfo, ImageInfo, VolumeInfo, NetworkInfo};
...
impl App {
    ...
    fn update_ui_from_detectors(&mut self) {
        if let Some(ref wsl_detector) = self.wsl_detector {
            self.ui_state.wsl.distributions = wsl_detector
                .distributions()
                .keys()
                .cloned()
                .collect();
    ...
    fn handle_main_menu_key(&mut self, key: crossterm::event::KeyEvent) {
        match key.code {
            ...
            KeyCode::Down => {
                let selected = self.ui_state.main_menu.selected().unwrap_or(0);
                if selected < 7 { // 8 menu items (0-7)
                    self.ui_state.main_menu.select(Some(selected + 1));
                }
            }
            KeyCode::Enter => {
    ...
    fn handle_wsl_key(&mut self, key: crossterm::event::KeyEvent) {
        match key.code {
            ...
            KeyCode::Down => {
                if self.ui_state.wsl.selected_dist < self.ui_state.wsl.distributions.len().saturating_sub(1) {
                    self.ui_state.wsl.selected_dist += 1;
                }
            }
            KeyCode::Char('c') => {
    ...
    fn handle_docker_key(&mut self, key: crossterm::event::KeyEvent) {
        match key.code {
            ...
            KeyCode::Down => {
                ...
                match self.ui_state.docker.selected_tab {
                    0 => {
                        if self.ui_state.docker.selected_container < self.ui_state.docker.containers.len().saturating_sub(1) {
                            self.ui_state.docker.selected_container += 1;
                        }
                    }
                    1 => {
                        if self.ui_state.docker.selected_image < self.ui_state.docker.images.len().saturating_sub(1) {
                            self.ui_state.docker.selected_image += 1;
                        }
                    }
                    2 => {
                        if self.ui_state.docker.selected_volume < self.ui_state.docker.volumes.len().saturating_sub(1) {
                            self.ui_state.docker.selected_volume += 1;
                        }
                    }
                    3 => {
                        if self.ui_state.docker.selected_network < self.ui_state.docker.networks.len().saturating_sub(1) {
                            self.ui_state.docker.selected_network += 1;
                        }
                    }
                    _ => {}
    ...
    fn handle_services_key(&mut self, key: crossterm::event::KeyEvent) {
        match key.code {
            ...
            KeyCode::Down => {
                if self.ui_state.services.selected_service < self.ui_state.services.services.len().saturating_sub(1) {
                    self.ui_state.services.selected_service += 1;
                }
            }
            KeyCode::Char('s') => {
    ...
    fn handle_package_managers_key(&mut self, key: crossterm::event::KeyEvent) {
        match key.code {
            ...
            KeyCode::Down => {
                if self.ui_state.package_managers.selected_manager < self.ui_state.package_managers.managers.len().saturating_sub(1) {
                    self.ui_state.package_managers.selected_manager += 1;
                }
            }
            KeyCode::Char('c') => {
    ...
    fn handle_about_key(&mut self, key: crossterm::event::KeyEvent) {
        match key.code {
            ...
            KeyCode::Up => {
                self.ui_state.about.scroll_offset = self.ui_state.about.scroll_offset.saturating_sub(1);
            }
            KeyCode::Down => {
                self.ui_state.about.scroll_offset = self.ui_state.about.scroll_offset.saturating_add(1);
            }
            _ => {}
        }
    }
    ...
    fn start_scan(&mut self) {
        ...
        info!("Starting scan with {} paths", self.ui_state.scan.paths.len());
    }
    
    /// Compact WSL distribution
    fn compact_wsl_distribution(&mut self) {
        if let Some(dist) = self.ui_state.wsl.distributions.get(self.ui_state.wsl.selected_dist) {
    ...
    fn cleanup_windows_update(&mut self) {
        ...
        self.ui_state.windows_update.status_message = "Cleaning Windows Update cache...".to_string();
        self.ui_state.progress.active = true;
    ...
    fn toggle_service(&mut self) {
        if let Some(service) = self.ui_state.services.services.get(self.ui_state.services.selected_service) {
    ...
    fn clean_selected_package_manager(&mut self) {
        if let Some(manager) = self.ui_state.package_managers.managers.get(self.ui_state.package_managers.selected_manager) {
            if manager.installed {
                info!("Cleaning {} cache", manager.name);
                self.ui_state.package_managers.cleaning = true;
                self.ui_state.package_managers.status_message = format!("Cleaning {}...", manager.display_name);
    ...
    fn clean_all_package_managers(&mut self) {
        ...
        self.ui_state.package_managers.cleaning = true;
        self.ui_state.package_managers.status_message = "Cleaning all package managers...".to_string();
    ...
    fn show_package_manager_info(&mut self) {
        if let Some(manager) = self.ui_state.package_managers.managers.get(self.ui_state.package_managers.selected_manager) {
            ...
            self.ui_state.package_managers.status_message = format!("Viewing {} cache details", manager.display_name);
    ...
    fn refresh_docker_resources(&mut self) {
        ...
        if self.docker_client.is_none() {
            self.docker_client = self.runtime.block_on(async {
        ...
        if let Some(ref client) = self.docker_client {
            ...
            if self.ui_state.docker.daemon_running {
                // Get containers
                self.ui_state.docker.containers = self.runtime.block_on(async {
                    client.get_containers().await.unwrap_or_default()
                });
                
                // Get images
                self.ui_state.docker.images = self.runtime.block_on(async {
                    client.get_images().await.unwrap_or_default()
                });
                
                // Get volumes
                self.ui_state.docker.volumes = self.runtime.block_on(async {
                    client.get_volumes().await.unwrap_or_default()
                });
                
                // Get networks
                self.ui_state.docker.networks = self.runtime.block_on(async {
                    client.get_networks().await.unwrap_or_default()
                });
            } else {
    ...
    fn delete_selected_docker_item(&mut self) {
        if let Some(ref client) = self.docker_client {
            match self.ui_state.docker.selected_tab {
                0 => { // Containers
                    if let Some(container) = self.ui_state.docker.containers.get(self.ui_state.docker.selected_container) {
                        info!("Removing container: {}", container.name);
                        
                        // Stop if running
                        if matches!(container.status, winsweep_core::docker::ContainerStatus::Running) {
                            if let Err(e) = self.runtime.block_on(client.stop_container(&container.id)) {
                                self.ui_state.docker.status_message = format!("Failed to stop container: {}", e);
                                return;
                            }
                        }
                        
                        // Remove container
                        if let Err(e) = self.runtime.block_on(client.remove_container(&container.id, false)) {
                            self.ui_state.docker.status_message = format!("Failed to remove container: {}", e);
                        } else {
                            self.ui_state.docker.status_message = format!("Removed container: {}", container.name);
                            // Refresh list
                            self.refresh_docker_resources();
                        }
                    }
                ...
                1 => { // Images
                    if let Some(image) = self.ui_state.docker.images.get(self.ui_state.docker.selected_image) {
                        info!("Removing image: {}", image.repository);
                        
                        if let Err(e) = self.runtime.block_on(client.remove_image(&image.id, false)) {
                            self.ui_state.docker.status_message = format!("Failed to remove image: {}", e);
                        } else {
                            self.ui_state.docker.status_message = format!("Removed image: {}", image.repository);
                            // Refresh list
                            self.refresh_docker_resources();
                        }
                    }
                ...
                2 => { // Volumes
                    if let Some(volume) = self.ui_state.docker.volumes.get(self.ui_state.docker.selected_volume) {
                        info!("Removing volume: {}", volume.name);
                        
                        if let Err(e) = self.runtime.block_on(client.remove_volume(&volume.name, false)) {
                            self.ui_state.docker.status_message = format!("Failed to remove volume: {}", e);
                        } else {
                            self.ui_state.docker.status_message = format!("Removed volume: {}", volume.name);
                            // Refresh list
                            self.refresh_docker_resources();
                        }
                    }
                ...
                3 => { // Networks
                    if let Some(network) = self.ui_state.docker.networks.get(self.ui_state.docker.selected_network) {
                        // Skip default networks
                        if network.name == "bridge" || network.name == "host" || network.name == "none" {
                            self.ui_state.docker.status_message = "Cannot remove default network".to_string();
                            return;
                        }
                        ...
                        if let Err(e) = self.runtime.block_on(client.remove_network(&network.name)) {
                            self.ui_state.docker.status_message = format!("Failed to remove network: {}", e);
                        } else {
                            self.ui_state.docker.status_message = format!("Removed network: {}", network.name);
                            // Refresh list
                            self.refresh_docker_resources();
                        }
                    }
            ...
            match self.ui_state.docker.selected_tab {
                0 => { // Containers
                    ...
                    for container in containers {
                        ...
                        // Remove container
                        if self.runtime.block_on(client.remove_container(&container.id, true)).is_ok() {
                            removed += 1;
                        }
                    }
                ...
                1 => { // Images
                    ...
                    for image in images {
                        ...
                        if image.dangling {
                            if self.runtime.block_on(client.remove_image(&image.id, true)).is_ok() {
                                removed += 1;
                            }
                        }
                    }
                    ...
                    self.ui_state.docker.cleaning = false;
                    self.ui_state.docker.status_message = format!("Removed {} dangling images", removed);
                    self.refresh_docker_resources();
                }
                2 => { // Volumes
                    ...
                    for volume in volumes {
                        if self.runtime.block_on(client.remove_volume(&volume.name, true)).is_ok() {
                            removed += 1;
                        }
                    }
                ...
                3 => { // Networks
                    ...
                    for network in networks {
                        ...
                        if self.runtime.block_on(client.remove_network(&network.name)).is_ok() {
                            removed += 1;
                        }
                    }
    ...
    fn draw_header(&self, f: &mut Frame<CrosstermBackend<std::io::Stdout>>, area: Rect) {
        let header_text = vec![
            Line::from(vec![
                Span::styled("WinSweep", Style::default().fg(Color::Cyan).add_modifier(Modifier::BOLD)),
                Span::raw(" - "),
                Span::styled("Windows Disk Cleaning Tool", Style::default().fg(Color::Gray)),
            ])
        ];
    ...
    fn draw_scan_page(&self, f: &mut Frame<CrosstermBackend<std::io::Stdout>>, area: Rect) {
        ...
        let paragraph = Paragraph::new(text)
            .block(Block::default().borders(Borders::ALL).title("System Scan"));
    ...
    fn draw_wsl_page(&self, f: &mut Frame<CrosstermBackend<std::io::Stdout>>, area: Rect) {
        ...
        let items: Vec<ListItem> = self.ui_state.wsl.distributions
            .iter()
            .enumerate()
            .map(|(i, dist)| {
                ...
                let version = if let Some(ref wsl_detector) = self.wsl_detector {
                    wsl_detector.get_distribution(dist)
                        .map(|d| format!(" (WSL{})", match d.version {
                            WslVersion::Wsl1 => "1",
                            WslVersion::Wsl2 => "2",
                            WslVersion::Unknown => "?",
                        }))
        ...
        let dist_list = List::new(items)
            .block(Block::default().borders(Borders::ALL).title("Distributions"))
            ...
            if let Some(ref wsl_detector) = self.wsl_detector {
                if let Some(distro_info) = wsl_detector.get_distribution(dist) {
                    ...
                    info_text.push(Line::from(vec![
                        ...
                        Span::styled(match distro_info.version {
                    ...
                    info_text.push(Line::from(vec![
                        ...
                        Span::styled(match distro_info.state {
                            WslState::Running => "Running",
                            WslState::Stopped => "Stopped",
                    ...
                    if distro_info.version == WslVersion::Wsl2 {
                        if let Ok(size) = wsl_detector.get_vhdx_size(dist) {
                            ...
                            info_text.push(Line::from(vec![
                                ...
                                Span::styled(format!("{:.2} MB", size as f64 / 1024.0 / 1024.0), Style::default()),
                            ]));
                        }
                    }
                }
        ...
        let status_text = if self.ui_state.wsl.compacting {
            format!("Compacting... {:.1}%", self.ui_state.wsl.compact_progress * 100.0)
    ...
    fn draw_docker_page(&mut self, f: &mut Frame<CrosstermBackend<std::io::Stdout>>, area: Rect) {
            ...
            0 => { // Containers
                if self.ui_state.docker.containers.is_empty() {
                    ...
                    for (i, container) in self.ui_state.docker.containers.iter().enumerate() {
                        ...
                        let size_text = if let (Some(rw), Some(root)) = (container.size_rw, container.size_root_fs) {
            ...
            1 => { // Images
                if self.ui_state.docker.images.is_empty() {
                    ...
                    for (i, image) in self.ui_state.docker.images.iter().enumerate() {
                        ...
                        let tag = if image.tag == "<none>" { "<dangling>" } else { &image.tag };
                        let text = format!("{}:{} ({}B)", image.repository, tag, format_bytes(image.size));
        ...
        let list = List::new(items)
            .block(Block::default().borders(Borders::ALL).title(match self.ui_state.docker.selected_tab {
                0 => "Containers",
                1 => "Images",
                2 => "Volumes",
                3 => "Networks",
        ...
        let status_text = if self.ui_state.docker.cleaning {
            format!("Cleaning Docker... Space freed: {}", format_bytes(self.ui_state.docker.space_freed))
    ...
    fn draw_windows_update_page(&self, f: &mut Frame<CrosstermBackend<std::io::Stdout>>, area: Rect) {
        let chunks = Layout::default()
            .direction(Direction::Vertical)
            .constraints([
                Constraint::Length(3), // Title
        ...
        info_text.push(Line::from("You may not be able to uninstall updates after cleanup."));
        info_text.push(Line::from(""));
    ...
    fn draw_services_page(&self, f: &mut Frame<CrosstermBackend<std::io::Stdout>>, area: Rect) {
        ...
        if self.ui_state.services.services.is_empty() {
            ...
            for (i, service) in self.ui_state.services.services.iter().enumerate() {
                ...
                let span = if i == self.ui_state.services.selected_service {
                    vec![
                        Span::styled(text, Style::default().fg(status_color).add_modifier(Modifier::BOLD))
                    ]
                } else {
                    vec![
                        Span::styled(text, Style::default().fg(status_color))
                    ]
                };
        ...
        let status_text = if self.ui_state.services.managing {
            "Managing service..."
        } else if let Some(service) = self.ui_state.services.services.get(self.ui_state.services.selected_service) {
            format!(
                "Actions: [s] Start/Stop, [r] Restart, [Enter] Properties - {}",
                if service.can_stop { "Can stop" } else { "Cannot stop" }
            )
        } else {
    ...
    fn draw_package_managers_page(&mut self, f: &mut Frame<CrosstermBackend<std::io::Stdout>>, area: Rect) {
        let chunks = Layout::default()
            .direction(Direction::Vertical)
            .constraints([
                Constraint::Length(3), // Title
        ...
        let items: Vec<ListItem> = self.ui_state.package_managers.managers
            .iter()
            .enumerate()
            .map(|(i, manager)| {
        ...
        let list = List::new(items)
            .block(Block::default().borders(Borders::ALL).title("Installed Package Managers"))
            .highlight_style(Style::default().add_modifier(Modifier::BOLD));
        ...
        if let Some(manager) = self.ui_state.package_managers.managers.get(self.ui_state.package_managers.selected_manager) {
            info_text.push(Line::from(vec![
                Span::styled("Selected: ", Style::default().fg(Color::Gray)),
                Span::styled(&manager.display_name, Style::default().add_modifier(Modifier::BOLD)),
            ]));
            ...
            info_text.push(Line::from(vec![
                ...
                Span::styled(
                    if manager.installed { "Installed" } else { "Not Installed" },
                    Style::default().fg(if manager.installed { Color::Green } else { Color::Red })
                ),
            ]));
    ...
    fn draw_config_page(&self, f: &mut Frame<CrosstermBackend<std::io::Stdout>>, area: Rect) {
        ...
        items.push(ListItem::new(Line::from(vec![
            Span::styled("Scanner", Style::default().add_modifier(Modifier::BOLD).fg(Color::Yellow)),
        ])));
        ...
        items.push(ListItem::new(Line::from(vec![
            ...
            Span::styled(
                if self.config.scan.include_hidden { "Yes" } else { "No" },
                Style::default().fg(Color::White)
            ),
        ])));
        ...
        items.push(ListItem::new(Line::from(vec![
            ...
            Span::styled(
                if self.config.scan.follow_symlinks { "Yes" } else { "No" },
                Style::default().fg(Color::White)
            ),
        ])));
        ...
        // Cleanup settings
        items.push(ListItem::new(Line::from(vec![
            Span::styled("Cleanup", Style::default().add_modifier(Modifier::BOLD).fg(Color::Yellow)),
        ])));
        
        items.push(ListItem::new(Line::from(vec![
            Span::styled("  Use Recycle Bin: ", Style::default().fg(Color::Gray)),
            Span::styled(
                if self.config.cleanup.use_recycle_bin { "Yes" } else { "No" },
                Style::default().fg(Color::White)
            ),
        ])));
        ...
        items.push(ListItem::new(Line::from(vec![
            ...
            Span::styled(
                if self.config.cleanup.create_restore_point { "Yes" } else { "No" },
                Style::default().fg(Color::White)
            ),
        ])));
        ...
        // UI settings
        items.push(ListItem::new(Line::from(vec![
            Span::styled("UI", Style::default().add_modifier(Modifier::BOLD).fg(Color::Yellow)),
        ])));
