...
use winsweep_core::{DockerClient, ContainerInfo, ImageInfo, VolumeInfo, NetworkInfo};
...
impl DockerViewModel {
    ...
    pub async fn refresh_resources(&mut self, docker_client: &DockerClient) -> Result<(), Box<dyn std::error::Error>> {
    ...
    pub async fn clean_selected(&mut self, docker_client: &DockerClient) -> Result<(), Box<dyn std::error::Error>> {
    ...
    /// Clean all resources
    pub async fn clean_all(&mut self, docker_client: &DockerClient) -> Result<(), Box<dyn std::error::Error>> {
        self.operation_in_progress = true;
        self.operation_progress = 0.0;
