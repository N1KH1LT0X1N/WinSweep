...
use winsweep_core::{PackageManagerRegistry, PackageManager, PackageCleanResult, CacheInfo};
...
impl PackageManagersViewModel {
    ...
    pub async fn refresh_managers(&mut self, registry: &PackageManagerRegistry) -> Result<(), Box<dyn std::error::Error>> {
        self.managers.clear();
    ...
    pub async fn clean_selected(&mut self, registry: &PackageManagerRegistry) -> Result<PackageCleanResult, Box<dyn std::error::Error>> {
        if let Some(index) = self.selected_manager {
            if index < self.managers.len() {
                ...
                self.status_message = Some(format!("Cleaning {} cache...", self.managers[index].display_name));
    ...
    pub async fn clean_all(&mut self, registry: &PackageManagerRegistry) -> Result<Vec<PackageCleanResult>, Box<dyn std::error::Error>> {
        self.operation_in_progress = true;
        self.operation_progress = 0.0;
