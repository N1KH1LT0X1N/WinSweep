        ...
        egui::CentralPanel::default().show(ctx, |ui| {
            ...
            ui.horizontal(|ui| {
                ...
                ui.vertical(|ui| {
                    ...
                    if ui.selectable_label(current_view == NavigationView::Dashboard, "📊 Dashboard")
                        .clicked() {
                        self.viewmodel.set_current_view(NavigationView::Dashboard);
                    }
                    
                    if ui.selectable_label(current_view == NavigationView::Scan, "🔍 System Scan")
                        .clicked() {
                        self.viewmodel.set_current_view(NavigationView::Scan);
                    }
                    
                    if ui.selectable_label(current_view == NavigationView::Wsl, "🐧 WSL Management")
                        .clicked() {
                        self.viewmodel.set_current_view(NavigationView::Wsl);
                    }
                    
                    if ui.selectable_label(current_view == NavigationView::Docker, "🐳 Docker Cleanup")
                        .clicked() {
                        self.viewmodel.set_current_view(NavigationView::Docker);
                    }
                    
                    if ui.selectable_label(current_view == NavigationView::PackageManagers, "📦 Package Managers")
                        .clicked() {
                        self.viewmodel.set_current_view(NavigationView::PackageManagers);
                    }
                    
                    if ui.selectable_label(current_view == NavigationView::WindowsUpdate, "🔄 Windows Update")
                        .clicked() {
                        self.viewmodel.set_current_view(NavigationView::WindowsUpdate);
                    }
                    
                    if ui.selectable_label(current_view == NavigationView::Services, "⚙️ Services")
                        .clicked() {
                        self.viewmodel.set_current_view(NavigationView::Services);
                    }
                    
                    if ui.selectable_label(current_view == NavigationView::Settings, "📝 Settings")
                        .clicked() {
                ...
                // Main content
                ui.vertical(|ui| {
                    match self.viewmodel.current_view() {
