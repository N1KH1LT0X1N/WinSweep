...
pub fn show_windows_update(ui: &mut egui::Ui, viewmodel: &mut WinSweepViewModel) {
    ...
    ui.horizontal(|ui| {
        ...
        ui.label(format!(
            "Last check: {}",
            viewmodel.windows_update.update_status.last_check
        ));
        ui.label(format!(
            "Pending updates: {}",
            viewmodel.windows_update.update_status.pending_updates
        ));
    });
    ...
    ui.horizontal(|ui| {
        ...
        if viewmodel.windows_update.cleanup_in_progress {
            ...
            ui.add(egui::ProgressBar::new(
                viewmodel.windows_update.cleanup_progress,
            ));
        }
    });
    ...
    ui.horizontal(|ui| {
        ...
        ui.label(format_bytes(
            viewmodel.windows_update.update_status.download_size,
        ));
    });
                    ...
                    frame.show(ui, |ui| {
                        ui.horizontal(|ui| {
                            ...
                            let (color, icon) = match update.category {
                                crate::viewmodel::windows_update::UpdateCategory::Critical => {
                                    (egui::Color32::RED, "🔴")
                                }
                                crate::viewmodel::windows_update::UpdateCategory::Important => {
                                    (egui::Color32::YELLOW, "🟡")
                                }
                                crate::viewmodel::windows_update::UpdateCategory::Optional => {
                                    (egui::Color32::GRAY, "⚪")
                                }
                                crate::viewmodel::windows_update::UpdateCategory::Driver => {
                                    (egui::Color32::BLUE, "🔵")
                                }
                            };
    ...
    ui.checkbox(
        &mut viewmodel.windows_update.cleanup_options.remove_downloads,
        "Remove downloaded updates",
    );
    ui.checkbox(
        &mut viewmodel.windows_update.cleanup_options.compress_backups,
        "Compress backup files",
    );
    ui.checkbox(
        &mut viewmodel.windows_update.cleanup_options.remove_old_versions,
