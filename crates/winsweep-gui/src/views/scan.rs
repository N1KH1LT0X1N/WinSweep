...
pub fn show_scan(ui: &mut egui::Ui, viewmodel: &mut WinSweepViewModel) {
    ...
    ui.horizontal(|ui| {
        ui.checkbox(&mut viewmodel.scan.scan_options.include_hidden, "Include hidden files");
        ui.checkbox(&mut viewmodel.scan.scan_options.include_system, "Include system files");
        
        ui.label("Min file size:");
        ui.add(egui::DragValue::new(&mut viewmodel.scan.scan_options.min_file_size)
            .suffix(" bytes"));
    });
    ...
    ui.horizontal(|ui| {
        if viewmodel.scan.scan_in_progress {
            ...
            ui.add(egui::ProgressBar::new(viewmodel.scan.scan_progress)
                .text(format!("Scanning... {:.1}%", viewmodel.scan.scan_progress * 100.0)));
        } else {
