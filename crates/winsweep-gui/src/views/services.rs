                    ...
                    frame.show(ui, |ui| {
                        ui.horizontal(|ui| {
                            ...
                            ui.with_layout(egui::Layout::right_to_left(egui::Align::Center), |ui| {
                                if service.can_stop && matches!(service.status, winsweep_core::ServiceStatus::Running) {
                                    if ui.button("⏹").clicked() {
                                        // TODO: Stop service
                                    }
                                }
                                
                                if service.can_start && !matches!(service.status, winsweep_core::ServiceStatus::Running) {
                                    if ui.button("▶").clicked() {
                                        // TODO: Start service
                                    }
                                }
                        ...
                        if ui.allocate_response(egui::Vec2::ZERO, egui::Sense::click()).clicked() {
                            ...
                            viewmodel.services.selected_service = viewmodel.services.services
                                .iter()
