    ...
    if !viewmodel.package_managers.managers.is_empty() {
                ...
                for (i, manager) in viewmodel.package_managers.managers.iter().enumerate() {
                    ...
                    frame.show(ui, |ui| {
                        ui.horizontal(|ui| {
                            ...
                            ui.colored_label(status_color, if manager.installed { "✓" } else { "✗" });
                            
                            // Manager info
                            ui.vertical(|ui| {
                                ui.label(&manager.display_name);
                                ui.label(format!("Version: {}", manager.version.as_deref().unwrap_or("Not installed")));
                                ui.label(format!("Cache size: {}", format_bytes(manager.cache_size)));
                            });
        ...
        ui.horizontal(|ui| {
            if viewmodel.package_managers.operation_in_progress {
                ui.spinner();
                ui.add(egui::ProgressBar::new(viewmodel.package_managers.operation_progress));
            } else {
